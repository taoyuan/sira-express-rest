...
var debug = require('debug')('sira:rest');
...
exports = module.exports = function (remotes, handler) {
    if (remotes.remotes && remotes.handle) {
        handler = handler || remotes;
        remotes = remotes.remotes;
    }
    var rest = new Rest(remotes, handler);
...
function Rest(remotes, handler) {
    EventEmitter.call(this);

    this.remotes = remotes;
    this.handler = handler && (typeof handler === 'function' ? handler : _.bind(handler.handle, handler));
}
...
Rest.prototype._registerMethodRouteHandlers = function(router, sharedMethod, route) {
    ...
    router[verb](route.path, function (req, res, next) {
        var context = new RestContext(req, res, self.handler);
        self._handleRequest(context, sharedMethod, next);
    });
};

Rest.prototype._handleRequest = function(context, method, next) {
    context.handle(method, function (err) {
...
function RestContext(req, res, handler) {
    this.req = req;
    this.res = res;
    this.handler = handler;
}

RestContext.prototype._buildRequest = function (method) {
    return sira.rekuest(method)
        .params(_.assign({}, this.req.query, this.req.params))
        .payload(this.req.body)
        .prop('body', this.req.body)
        .prop('query', this.req.query)
        .prop('path', this.req.params)
        .prop('accessToken', this.req.accessToken)
        .req;
};

RestContext.prototype.handle = function (method, cb) {
    var self = this, req = this.req;
    var future = this.handler(this._buildRequest(method), function (err, ctx) { // sira context
        if (err === 'canceled') return debug('request has been canceled - ' + req.url);
        if (err) return cb(err);
        self.done(ctx.result);
        cb();
    });

    if (future) {
        this.req.on('close',future.cancel);
    }
};

RestContext.prototype.done = function (data) {
