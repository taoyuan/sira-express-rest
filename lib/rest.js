...
Rest.buildClasses = function (sappOrRemotes) {
    return (sappOrRemotes.remotes || sappOrRemotes).classes().map(RestClass);
};
...
RestContext.prototype.handle = function (method, cb) {
    ...
    var future = this.sapp.rekuest(method).props(req).payload(req.body)
