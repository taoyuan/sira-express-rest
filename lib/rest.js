...
RestContext.prototype.handle = function (method, cb) {
    var self = this, req = this.req, canceled;

    // future may be null if some sira middleware are async
    var future = this.handler(this._buildRequest(method), function (err, ctx) { // sira context
        if (err === 'canceled' || canceled) return debug('request has been canceled - ' + req.url);
